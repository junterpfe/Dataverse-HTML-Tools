...
- Importing a Power Platform admin export when the Maker/Admin app list includes assets not visible in Dataverse tables
...
| Admin export JSON | Imports JSON generated from `Get-AdminPowerApp` and `Get-AdminFlow` so the inventory can include Power Apps service assets that do not appear in Dataverse app tables |
...
If Power Platform Admin Center shows more apps than Environment inventory returns, those extra assets are not visible through this same-environment Dataverse web resource. This is common in Default environments with many personal productivity canvas apps. Use **Admin export JSON** with Power Platform admin APIs or PowerShell, such as `Get-AdminPowerApp`, for full Admin Center coverage.

### Admin export JSON

Use this mode when the Maker/Admin app list has apps that do not appear in the Dataverse-visible inventory. Click **Copy export script**, run the script in PowerShell with appropriate Power Platform admin permissions, then load the generated `power-platform-admin-inventory.json` file.

The imported export can show all apps/flows returned by the admin cmdlets, plus any service/URL metadata included in the exported objects. It does not prove solution package membership unless that membership is present in the export.
...
| Environment inventory count is lower than Maker/Admin app list | Use **Admin export JSON** from `Get-AdminPowerApp` / `Get-AdminFlow`; those missing apps are outside the Dataverse-visible inventory |
