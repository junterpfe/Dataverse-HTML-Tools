...
For inherited roles, the source is labeled as `Team`, `Entra security group`, or `Microsoft 365 group` from the related Dataverse team type. This surfaces permissions synchronized from group-backed Dataverse teams without requiring Microsoft Graph access. When Dataverse provides the group's Entra object ID, the Entra security group and Microsoft 365 group pills open that group in the Entra portal: `entra.microsoft.com` for Commercial and standard GCC, and `entra.microsoft.us` for GCC High and DoD.
...
Admins can search for another user and select which assignments to copy:

- Direct security role assignments
- Removable owner-team memberships
- Record-specific access-team memberships

Direct roles and owner-team memberships are selected by default. Access-team memberships are optional because they transfer record-specific access. Microsoft Entra group membership is not included because it must be managed in Entra ID.

The confirmation dialog provides two modes: `Add` keeps existing selected assignments and adds missing source assignments, while `Clone` removes existing assignments in the selected categories before applying the source assignments. For roles, the tool attempts to map the source user's roles to matching roles in the target user's business unit.
...
- Entra security group and Microsoft 365 group permissions are visible when Dataverse has synchronized the user's membership to a corresponding group team. The tool cannot enumerate groups that are not represented by a Dataverse team.
